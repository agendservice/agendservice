...
const buscar = () => {
  loader.value = true;
  axios.post('/cliente', resultados.filtros)
    .then(({ data }) => {
      atualizarResultados({
        ...data,
        filtros: resultados.filtros
    ...
    .catch(() => {
      erro.value = true;
      mensagem.value = 'Erro ao buscar clientes.';
    })
    .finally(() => {
      loader.value = false;
    });
...
const submit = (model) => {
  ...
  const promise = model.id
    ? axios.put('/cliente/atualizar', model)
    : axios.post('/cliente/cadastrar', model);
