<template>
  <app-intranet
    icone="mdi-calendar-clock-outline"
...
const colunas = [
  ...
  { text: 'Início', value: 'data_hora_inicio', type: 'data' },
...
const buscar = () => {
  loader.value = true;
  axios.post('/agendamento', resultados.filtros)
    .then(({ data }) => {
      atualizarResultados({
        ...data,
        filtros: resultados.filtros
...
const submit = (model) => {
  ...
  const promise = model.id
    ? axios.put('/agendamento/atualizar', model)
    : axios.post('/agendamento/cadastrar', model);
...
const remover = (model) => {
  loader.value = true;
  axios.delete('/agendamento/' + model.id)
    .then(() => {
      tipo.value = 'success';
